...
<head>
    ...
    <title>Login - FoodDetect Admin</title>
...
<body class="font-sans text-gray-800 antialiased h-screen flex login-bg">
    ...
    <div class="m-auto w-full max-w-md">
        <div class="text-center mb-8">
            ...
            <p class="text-white/60 text-sm mt-2">Sistem Deteksi Kontaminasi Makanan IoT</p>
        </div>

        <div class="bg-white rounded-2xl shadow-2xl p-8">
            <h2 class="text-2xl font-bold text-brandDark mb-6 text-center">Login Admin</h2>
            
            <form action="{{ url('/login') }}" method="POST" class="space-y-5">
                @csrf
                <div>
                    <label class="block text-sm font-semibold text-gray-600 mb-1">Email / Username</label>
                    <input type="email" name="email" class="w-full px-4 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-brandGreen outline-none" placeholder="admin@fooddetect.id" required>
                </div>
            ...
            <div class="mt-6 text-center text-sm text-gray-500">
                Belum punya akun? <a href="{{ route('register') }}" class="text-brandGreen font-semibold hover:underline">Daftar Sekarang</a>
            </div>
        </div>
    </div>
